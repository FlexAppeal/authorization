...
const beforeEach = () => sandbox = sinon.sandbox.create();
const afterEach = () => sandbox.restore();
...
test('should return true when user has the right permission for an action', t => {
  beforeEach();
  sandbox.stub(authorization, 'getUserRole').returns('admin');
  sandbox.stub(authorization, 'getActionByKey').returns(['employee', 'admin']);
...
test('should return false when user doesn\'t have the right permission for an action', t => {
  beforeEach();
  sandbox.stub(authorization, 'getUserRole').returns('employee');
  sandbox.stub(authorization, 'getActionByKey').returns({ role: 'admin' });
...
test('should return true when user doesn\'t have the right permission for an action but the "or" function returns true', t => {
  beforeEach();
  sandbox.stub(authorization, 'getUserRole').returns('employee');
  sandbox.stub(authorization, 'getActionByKey').returns({ role: 'admin', or: () => true });
...
test('should return true when user doesn\'t have the right permission for an action but the "and" function returns true', t => {
  beforeEach();
  sandbox.stub(authorization, 'getUserRole').returns('employee');
  sandbox.stub(authorization, 'getActionByKey').returns({ role: 'admin', and: () => true });
  const exchange = { id: 1, title: 'Test shift', user: { id: 1 } };
...
test('should throw an error when user is not authorized', t => {
  beforeEach();
  sandbox.stub(authorization, 'can').returns(false);
